...
static unsigned int GetTickCount() { return get_jiffiess(); }
void _getch(void) {}
int seatest_is_string_equal_i(const char *s1, const char *s2)
{
...
{
	if (seatest_suite_setup_func != 0)
		seatest_suite_setup_func();
}

void seatest_suite_teardown(void)
{
	if (seatest_suite_teardown_func != 0)
		seatest_suite_teardown_func();
}
...
void seatest_interpret_commandline(seatest_testrunner_t *runner)
{
	...
	for (arg = 0; (arg < runner->argc) && (runner->action != SEATEST_DO_ABORT); arg++)
	{
		...
		if (seatest_is_string_equal_i(runner->argv[arg], "-v"))
			seatest_verbose = 1;
		if (seatest_is_string_equal_i(runner->argv[arg], "-vs"))
			vs_mode = 1;
		if (seatest_is_string_equal_i(runner->argv[arg], "-m"))
			seatest_machine_readable = 1;
		if (seatest_parse_commandline_option_with_value(runner, arg, "-t", test_filter))
			arg++;
		if (seatest_parse_commandline_option_with_value(runner, arg, "-f", fixture_filter))
			arg++;
		if (seatest_parse_commandline_option_with_value(runner, arg, "-k", set_magic_marker))
			arg++;
	}
}
